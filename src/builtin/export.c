...
static void print_declare_env(t_cmd *cmd) {
  int i;
...
static void handle_export(t_cmd cmd, char *value, char *key) {
  int idx;
  char *old;
  char *tmp;
  char *new_var;
  ...
  if (idx != -1) {
    old = ft_substr(cmd.env[idx], ft_strlen(key) + 1, ft_strlen(cmd.env[idx]));
...
bool is_valid_var_id(t_cmd cmd) {
  int i;
