...
void exec_leon(t_list *cmd_list) {
    t_cmd *cmd = (t_cmd *)cmd_list->content;

    // Ensure cmd and cmd->cmd_table are not NULL
  ...
  while (cmd_list) {
    ...
    if (cmd && cmd->cmd_table) {
      if (path_verification(envp_path, cmd) == 0) {
        // Successfully updated cmd_table[0] with the full path
        printf("Updated command: %s\n", cmd->cmd_table[0]);
      } else {
        // Failed to update, handle accordingly
        printf("Command not found: %s\n", cmd->cmd_table[0]);
      }
    }
    cmd_list = cmd_list->next;
  }
  // Free the envp_path array when done
  i = 0;
  while (envp_path[i] != NULL) {
    free(envp_path[i]);
    i++;
